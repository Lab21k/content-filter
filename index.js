module.exports = function filter(options) {
	...
	return function filter(req, res, next) {
		...
		if (found) {
			return res.status(403).json({error: 'MALICIOUS_INPUT_ERROR', message: `${urlMessage} Found: ${found}`});
		}
		...
		if (req.body && Object.keys(req.body).length) {
			...
			jsonToString(req.body, typeList, checkNames, function(str){
				...
				if (found) {
					return res.status(403).json({error: 'MALICIOUS_INPUT_ERROR', message: `${bodyMessage} Found: ${found}`});
				}
				next();
			});
		} else {
